...
export default function SubjectPage() {
  ...
  const getAiAnswer = async (category, index) => {
    ...
    try {
      ...
      if (data.success) {
        setAiAnswers(prev => ({ ...prev, [key]: data.answer }));
      } else {
        console.error('AI API error:', data.error);
        setAiAnswers(prev => ({ ...prev, [key]: 'Sorry, unable to generate AI answer at the moment. Please try again later.' }));
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-100">
      ...
      <div className="max-w-7xl mx-auto px-6 pt-28">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-4">
              <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${currentSubject.color} flex items-center justify-center text-3xl shadow-lg`}>
                {currentSubject.icon}
              </div>
              <div>
        ...
        <div className="flex space-x-1 bg-gray-100 p-1 rounded-lg">
          {tabs.map((tab) => (
            <button
              ...
              className={`flex-1 flex items-center justify-center space-x-2 px-4 py-3 rounded-md font-medium transition-all duration-200 ${activeTab === tab.id ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50'}`}
            >
              <span className="text-lg">{tab.icon}</span>
              <span>{tab.name}</span>
              {tab.count !== null && (
                <span className={`px-2 py-1 rounded-full text-xs font-medium ${activeTab === tab.id ? 'bg-blue-100 text-blue-600' : 'bg-gray-200 text-gray-600'}`}>{tab.count}</span>
              )}
            </button>
          ))}
        </div>
      ...
      {/* Main Content */}
      {activeTab === "chat" ? (
        // Chat view below navbar
        <div className="pt-28 bg-white z-40 overflow-y-auto">
          <Chatbot />
        </div>
      ) : (
                        ...
                        <div className="flex items-start justify-between gap-4">
                          ...
                          <div className="flex items-start">
                            <button
                              ...
                              className={`px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 flex items-center gap-2 whitespace-nowrap ${loadingAiAnswers[`p-${i}`]
